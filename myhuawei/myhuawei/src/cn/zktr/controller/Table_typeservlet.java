...
/**
 * Servlet implementation class Table_typeservlet
 */
@WebServlet("/table_typeservlet")
public class Table_typeservlet extends BaseServlet {
	private static final long serialVersionUID = 1L;
	Hou_type_dao htdao=new Hou_type_dao();
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Table_typeservlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(admins==null) {
			...
			try {
					//获得总条数
					p.setTotal(htdao.typeselectByTotal(parma));
					//获得所有的数据
					p.setRows(htdao.typeselectByCurrent(current, p.getPageSize(),parma));
					//将整个对象保存到作用域中
					request.setAttribute("page", p);
					this.forward("hou/table_type");
			} catch (Exception e) {
