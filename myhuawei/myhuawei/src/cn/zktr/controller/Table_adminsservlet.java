 ...
 * Servlet 
 */
@WebServlet("/table_adminsservlet")
public class Table_adminsservlet extends BaseServlet {
	private static final long serialVersionUID = 1L;
	Hou_admins_dao hadao=new Hou_admins_dao();
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Table_adminsservlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(admins==null) {
			...
			try {
					//获得总条数
					p.setTotal(hadao.admins_tiao(parma));
					//获得所有的数据
					p.setRows(hadao.admins_paging(current, p.getPageSize(),parma));
					//将整个对象保存到作用域中
					request.setAttribute("page", p);
					this.forward("hou/table_admins");
			} catch (Exception e) {
