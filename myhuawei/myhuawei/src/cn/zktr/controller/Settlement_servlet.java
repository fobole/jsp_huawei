...
import cn.zktr.model.dao.Set_lijigoumai_dao;
...
public class Settlement_servlet extends BaseServlet {
	private static final long serialVersionUID = 1L;
	Qian_shopcar_cha_dao cdao=new Qian_shopcar_cha_dao();
	Set_order_dao sdao=new Set_order_dao();
	Set_lijigoumai_dao ldao=new Set_lijigoumai_dao();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			if(u==null){
				...
				System.out.println(userid+" "+shu+" "+conid);
				if( conid=="" || conid==null) {
					list = cdao.set_car(userid);
					//查询总金额和总件数
					goodsAmount = cdao.getAmount(userid);
					goodsNum = cdao.getTotal(userid);
					
					
				}else {
					 list=ldao.ligou(conid);
					//查询总金额和总件数
					goodsAmount = list.get(0).getConjia() * Integer.parseInt(shu);
					goodsNum = Integer.parseInt(shu);
					request.setAttribute("shu", shu);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if( conid=="" || conid==null || shu=="" || shu==null) {
				...
				Order_detail od=new Order_detail();
				od.setOnodo(ono);
				od.setConiddo(Integer.parseInt(conid));
				od.setDoshu(Integer.parseInt(shu));
				int orderDetails=sdao.insertOrderDetails(od);
			}
